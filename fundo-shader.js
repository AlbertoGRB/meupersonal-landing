// Fundo animado "Shader R" (Mesh drift) — portado do componente React de
// ola.leandroaraujo no 21st.dev para JavaScript puro. WebGL, zero dependências.
// Sem WebGL, mantém o fundo estático do body; com prefers-reduced-motion,
...
`;
  ...
  const CONFIG = {
    ...
    colorCount: 4,
    scale: 1.16, intensity: 0.34, paramA: 0.5, warp: 0.0,
    detail: 2.4, contrast: 1.158, brightness: 0.0, saturation: 1.0,
    hue: 0.0, vignette: 0.0, blur: 0.0, grain: 0.091,
    seed: 1453.0, rotate: 0.0, offsetX: 0.0, offsetY: 0.0, drift: 0.0,
  ...
  if (!gl) return; // sem WebGL: fica o fundo estático do body
  ...
  gl.attachShader(programa, compilar(gl.VERTEX_SHADER, VERT));
  gl.attachShader(programa, compilar(gl.FRAGMENT_SHADER, FRAG));
  gl.linkProgram(programa);
  if (!gl.getProgramParameter(programa, gl.LINK_STATUS)) { canvas.remove(); return; }
  ...
  const uni = {
    ...
    scene: gl.getUniformLocation(programa, 'u_scene'),
    shape: gl.getUniformLocation(programa, 'u_shape'),
    surface: gl.getUniformLocation(programa, 'u_surface'),
    finish: gl.getUniformLocation(programa, 'u_finish'),
    transform: gl.getUniformLocation(programa, 'u_transform'),
    space: gl.getUniformLocation(programa, 'u_space'),
  ...
  gl.uniform4f(uni.space, CONFIG.offsetX, CONFIG.offsetY, 0, 0);
  ...
  const redimensionar = () => {
    ...
    const larguraBruta = Math.max(1, Math.round(window.innerWidth * dpr));
    const alturaBruta = Math.max(1, Math.round(window.innerHeight * dpr));
    const escala = Math.min(1, Math.sqrt(2000000 / Math.max(1, larguraBruta * alturaBruta)));
    const largura = Math.max(1, Math.round(larguraBruta * escala));
    const altura = Math.max(1, Math.round(alturaBruta * escala));
